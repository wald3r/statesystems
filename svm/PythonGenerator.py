...
\n\
    pass\n\
\n\
  def handleEvent(self, se, params=[], cond=None):\n\
\n\
    return 0\n\
\n\
  def event(self, e, params=[], cond=None):\n\
\n\
    pass\n\
\n"
      dummy_init_model='\
...
\n\
    if self.Parent!=None:  # Non-top-level model\n\
      self.Parent.addSchedule(id, interval, event)\n\
      return\n\
...
\n\
    ...
    if self.action_ext:
      for k in keys:
        code=code+'\
    if table[%d] and self.isInState(%d) and self.Submodels[%d].handleEvent(se, params, cond):\n\
    ...
    if self.action_ext:
      code=code+'\
    if not handled and %s.RescheduleTable[e]>=0:\n\
      self.addSchedule(%s.RescheduleTable[e], %s.IntervalTable[e], %s.EventNames[e])\n' % (self.model_name, self.model_name, self.model_name, self.model_name)
    code=code+'\
    return handled\n'
    return code+"\n"
      ...
      if self.eventhandler.is_ifs(t['S']):
        ...
        if table[%d] and self.isInState(%d) and self.Submodels[%d].handleEvent(se, params, cond):\n\
          ...
          handled=1\n' % (k, k, k, self.model_name, k)
	        ...
	        code=code+'\
        if table[%d] and self.Submodels[%d].handleEvent(se, params, cond):\n\
          self.applyMask(%s.OrthogonalTable[%d], table)\n\
...
" "*space+'  self.Submodels[%d]=%s(self.DefaultInterpreter, self)\n' % (stnum, self.submodels[stnum]) + \
    ...
    for s in keys:
      ...
      for tt in tts:
        ...
        self.eventhandler.enter[s].append({'O': [
               '|# a timed transition',
               '|self.addSchedule(%s, %s.IntervalTable[%d], %s.EventNames[%d])' % (snum, self.model_name, evnum, self.model_name, evnum)]})
