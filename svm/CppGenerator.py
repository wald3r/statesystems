...
private:\n\
  ...
  static const bool    OrthogonalInBetween[[STATE_NUM]+1][[STATE_NUM]];\n\
  static const bool    OrthogonalTable[[STATE_NUM]][[STATE_NUM]];\n\
  static const char*   IntervalTable[[EVENT_NUM]];\n\
  static const int     RescheduleTable[[EVENT_NUM]];\n\
  static const int     CommonStateTable[[STATE_NUM]][[STATE_NUM]];\n\
  static const bool    Hierarchy[[STATE_NUM]][[STATE_NUM]];\n\
\n\
public:\n\
...
\n'
  ...
  StateMachine* Parent;\n'
      ...
      constructor_interface='\
  [MODEL_NAME](PyThreadState* interpreter=null, StateMachine* parent=null);\n'
      constructor_implementation='\
[MODEL_NAME]::[MODEL_NAME](PyThreadState* interpreter, StateMachine* parent) {\n\
  Parent=parent;\n\
  if (Parent!=null)\n\
...
protected:\n\
  ...
  virtual void addSchedule(int id, string interval, string event);\n\
  virtual void removeSchedule(int id);\n\
\n\
public:\n\
  ...
  virtual bool handleEvent(string se, PyObject* params=null, PyObject* pylock=null, pthread_mutex_t* lock=null, pthread_cond_t* cond=null);\n\
  virtual void event(string se, PyObject* params=null, PyObject* pylock=null, pthread_mutex_t* lock=null, pthread_cond_t* cond=null);\n'
      mdl_action_implementation='\
void* [MODEL_NAME]::handleEvent_wrapper(void* mdl) {\n\
  ...
  while (true) {\n\
    ...
    event->Next=null;\n\
    pthread_mutex_unlock(&model->EventsLock);\n\
    event->Callee->handleEvent(event->Event, event->Params, event->PyLock, event->Lock, event->Cond);\n\
    delete event;\n\
    pthread_mutex_lock(&model->EventsLock);\n\
    if (model->Stopped) {\n\
      pthread_mutex_unlock(&model->EventsLock);\n\
...
void [MODEL_NAME]::addSchedule(int id, string interval, string event) {\n\
  if (Parent!=null) {  // Non-top-level model\n\
    Parent->addSchedule(id, interval, event);\n\
    return;\n\
  }\n\
  // Compute interval\n\
  PyEval_AcquireLock();\n\
...
void [MODEL_NAME]::removeSchedule(int id) {\n\
  if (Parent!=null) {  // Non-top-level model\n\
    Parent->removeSchedule(id);\n\
    return;\n\
  }\n\
  pthread_mutex_lock(&SchedulerLock);\n\
  Schedule* this_sched;\n\
...
void* [MODEL_NAME]::scheduler(void* mdl) {\n\
  ...
  while (true) {\n\
    ...
    while (model->Schedules!=NULL && model->Schedules->time <= current_time) {\n\
      ...
      model->event(this_sched->event, null, null, &model->SchedulerLock, &wait_cond);\n\
...
void [MODEL_NAME]::event(string se, PyObject* params, PyObject* pylock, pthread_mutex_t* lock, pthread_cond_t* cond) {\n\
...
\n'
  ...
  virtual bool handleEvent(string se, PyObject* params=null, PyObject* pylock=null, pthread_mutex_t* lock=null, pthread_cond_t* cond=null)=0;\n\
  virtual void event(string se, PyObject* params=null, PyObject* pylock=null, pthread_mutex_t* lock=null, pthread_cond_t* cond=null)=0;\n\
  virtual void clearEnteredStates()=0;\n\
  virtual void addSchedule(int id, string interval, string event)=0;\n\
  virtual void removeSchedule(int id)=0;\n\
...
int main(int argn, char** argv) {\n\
  ...
  def generate_hierarchy_def(self):
    ...
    while ps<len(states):
      ...
      ps=ps+1
    return self.generate_array("const bool", "%s::Hierarchy[%d][%d]" % (self.model_name, self.state_num, self.state_num), codes, comments)

  def generate_event_code(self):
    if self.action_ext:
      code='\
bool [MODEL_NAME]::handleEvent(string se, PyObject* params, PyObject* pylock, pthread_mutex_t* lock, pthread_cond_t* cond) {\n\
  if (!Started) {\n\
...
bool [MODEL_NAME]::handleEvent(string se) {\n\
  ...
  switch (e) {\n' % (self.state_num, self.state_num)
    ...
    if self.action_ext:
      for k in keys:
        code=code+'\
  if (table[%d] && isInState(%d) && Submodels[%d]->handleEvent(se, params, pylock, lock, cond)) {\n\
  ...
  if (table[%d] && isInState(%d) && Submodels[%d]->handleEvent(se)) {\n\
    ...
    if self.action_ext:
      code=code+'\
  if (!handled && RescheduleTable[e]>=0)\n\
    addSchedule(RescheduleTable[e], IntervalTable[e], EventNames[e]);\n'
    code=code+'\
  return handled;\n\
}\n'
    return code
      ...
      if self.eventhandler.is_ifs(t['S']):
        ...
        if (table[%d] && isInState(%d) && Submodels[%d]->handleEvent(se, params, pylock, lock, cond)) {\n\
          ...
          handled=true;\n\
	        ...
	        code=code+'\
        if (table[%d] && Submodels[%d]->handleEvent(se, params, pylock, lock, cond)) {\n\
          applyMask(OrthogonalTable[%d], table);\n\
          handled=true;\n\
        }' % (k, k, k)
            else:
  ...
  def generate_in_states(self, stateH, com, des):
	...
	if self.submodels.has_key(stnum):
          ...
          if (history_type==1 && Submodels[%d]!=null)\n\
            ...
            Submodels[%d]->start();\n\
          }\n' % (stnum, stnum, stnum, stnum, stnum, stnum, stnum, self.submodels[stnum], self.submodels[stnum], self.submodels[stnum], stnum, stnum)
          else:
            code=code+'\
          if (history_type==1 && Submodels[%d]!=null)\n\
            Submodels[%d]->topLevelHistory();\n\
...
// Submodel Class "%s" -- the submodel executed by the top-level model' % eh.options[MODEL_NAME] + impl
  ...
  def generate_orthogonal_in_between_table(self):
	...
	i=i+1
      code=code+"}"
      codes.append(code)
    return self.generate_array("const bool", "%s::OrthogonalInBetween[%d+1][%d]" % (self.model_name, self.state_num, self.state_num), codes)
  ...
  def generate_orthogonal_table(self):
    ...
    while i<self.state_num:
      ...
      table[i]=s
      i=i+1
    return self.generate_array("const bool", "%s::OrthogonalTable[%d][%d]" % (self.model_name, self.state_num, self.state_num), table)
...
void %s::runExitActions(int state) {\n\
    ...
    for s in keys:
      ...
      for tt in tts:
        ...
        self.eventhandler.enter[s].append({'O': [
               '|// a timed transition',
               '|addSchedule(%s, IntervalTable[%d], EventNames[%d]);' % (snum, evnum, evnum)]})
        self.eventhandler.exit[s].append({'O': [
               '|// clean up timed transitions',
               '|removeSchedule(%s);' % snum]})
  ...
  def generate_interval_table(self):
    ...
    return self.generate_array("const char*", "%s::IntervalTable[%d]" % (self.model_name, self.event_num), table)
  ...
  def generate_reschedule_table(self):
    ...
    return self.generate_array("const int", "%s::RescheduleTable[%d]" % (self.model_name, self.event_num), table)
